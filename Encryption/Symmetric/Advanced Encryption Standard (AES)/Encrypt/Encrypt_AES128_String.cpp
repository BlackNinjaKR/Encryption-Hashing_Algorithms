#include <iostream>
#include <iomanip>
#include <cstdint>
#include <string>
#include <vector>

/* ================= AES CONSTANTS ================= */

static const uint8_t sBox[256] = {
    
};

static const uint8_t Rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

/* ================= AES CORE FUNCTIONS ================= */

uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        bool hi = a & 0x80;
        a <<= 1;
        if (hi) a ^= 0x1B;
        b >>= 1;
    }
    return p;
}

void SubBytes(uint8_t state[4][4]) {
    for (int r = 0; r < 4; r++)
        for (int c = 0; c < 4; c++)
            state[r][c] = sBox[state[r][c]];
}

void ShiftRows(uint8_t state[4][4]) {
    uint8_t t;

    t = state[1][0];
    state[1][0]=state[1][1]; state[1][1]=state[1][2];
    state[1][2]=state[1][3]; state[1][3]=t;

    t = state[2][0]; state[2][0]=state[2][2]; state[2][2]=t;
    t = state[2][1]; state[2][1]=state[2][3]; state[2][3]=t;

    t = state[3][3];
    state[3][3]=state[3][2]; state[3][2]=state[3][1];
    state[3][1]=state[3][0]; state[3][0]=t;
}

void MixColumns(uint8_t state[4][4]) {
    uint8_t t[4];
    for (int c = 0; c < 4; c++) {
        for (int i = 0; i < 4; i++) t[i] = state[i][c];

        state[0][c] = gmul(t[0],2)^gmul(t[1],3)^t[2]^t[3];
        state[1][c] = t[0]^gmul(t[1],2)^gmul(t[2],3)^t[3];
        state[2][c] = t[0]^t[1]^gmul(t[2],2)^gmul(t[3],3);
        state[3][c] = gmul(t[0],3)^t[1]^t[2]^gmul(t[3],2);
    }
}

void AddRoundKey(uint8_t state[4][4], uint8_t* roundKey) {
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[r][c] ^= roundKey[c*4 + r];
}

void KeyExpansion(uint8_t key[16], uint8_t expanded[176]) {
    for (int i = 0; i < 16; i++) expanded[i] = key[i];

    int bytes = 16, rcon = 1;
    uint8_t temp[4];

    while (bytes < 176) {
        for (int i = 0; i < 4; i++)
            temp[i] = expanded[bytes - 4 + i];

        if (bytes % 16 == 0) {
            uint8_t t = temp[0];
            temp[0]=temp[1]; temp[1]=temp[2];
            temp[2]=temp[3]; temp[3]=t;

            for (int i = 0; i < 4; i++)
                temp[i] = sBox[temp[i]];

            temp[0] ^= Rcon[rcon++];
        }

        for (int i = 0; i < 4; i++) {
            expanded[bytes] = expanded[bytes - 16] ^ temp[i];
            bytes++;
        }
    }
}

/* ================= AES ENCRYPT ================= */

void AES_encrypt(uint8_t in[16], uint8_t key[16], uint8_t out[16]) {
    uint8_t state[4][4], expanded[176];
    int i = 0;

    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[r][c] = in[i++];

    KeyExpansion(key, expanded);
    AddRoundKey(state, expanded);

    for (int round = 1; round <= 9; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, expanded + round * 16);
    }

    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, expanded + 160);

    i = 0;
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            out[i++] = state[r][c];
}

/* ================= Helpers ================= */

void deriveKeyFromString(const std::string& keyStr, uint8_t key[16]) {
    for (int i = 0; i < 16; i++) {
        if (i < (int)keyStr.size())
            key[i] = static_cast<uint8_t>(keyStr[i]);
        else
            key[i] = 0x00;
    }
}

std::vector<uint8_t> pkcs7Pad(const std::vector<uint8_t>& data) {
    size_t padLen = 16 - (data.size() % 16);
    std::vector<uint8_t> padded = data;

    for (size_t i = 0; i < padLen; i++)
        padded.push_back(static_cast<uint8_t>(padLen));

    return padded;
}

std::vector<uint8_t> AES_encrypt_ECB(
    const std::string& plaintext,
    const std::string& keyStr
) {
    uint8_t key[16];
    deriveKeyFromString(keyStr, key);

    std::vector<uint8_t> data(plaintext.begin(), plaintext.end());
    std::vector<uint8_t> padded = pkcs7Pad(data);

    std::vector<uint8_t> ciphertext(padded.size());

    uint8_t blockOut[16];

    for (size_t i = 0; i < padded.size(); i += 16) {
        AES_encrypt(&padded[i], key, blockOut);
        for (int j = 0; j < 16; j++)
            ciphertext[i + j] = blockOut[j];
    }

    return ciphertext;
}

void printHexVec(const std::vector<uint8_t>& data) {
    for (uint8_t b : data)
        std::cout << std::hex << std::setw(2)
                  << std::setfill('0') << (int)b;
    std::cout << std::dec << std::endl;
}

int main() {
    std::string plaintext;
    std::string key;

    std::cout << "Enter plaintext: ";
    std::getline(std::cin, plaintext);

    std::cout << "Enter key: ";
    std::getline(std::cin, key);

    auto ciphertext = AES_encrypt_ECB(plaintext, key);

    std::cout << "\nCiphertext (hex): ";
    printHexVec(ciphertext);

    return 0;
}
