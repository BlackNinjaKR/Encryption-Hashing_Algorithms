#include <iostream>
#include <iomanip>
#include <cstdint>
#include <cstring>

using byte = uint8_t;

/* ============================================================
   AES PARAMETERS (AES-128)
   ============================================================ */
#define Nb 4   // block size (columns)
#define Nk 4   // key size (words)
#define Nr 10  // number of rounds

/* ============================================================
   AES CONSTANT TABLES
   ============================================================ */

/* Inverse S-Box */
static const byte inv_sbox[256] = {
    
};

/* ============================================================
   GALOIS FIELD MULTIPLICATION
   ============================================================ */
byte gmul(byte a, byte b) {
    byte p = 0;
    while (b) {
        if (b & 1) p ^= a;
        a = (a << 1) ^ ((a & 0x80) ? 0x1B : 0);
        b >>= 1;
    }
    return p;
}

/* ============================================================
   AES ROUND OPERATIONS (INVERSE)
   ============================================================ */

void AddRoundKey(byte state[4][4], const byte* roundKey) {
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            state[r][c] ^= roundKey[c * 4 + r];
}

void InvSubBytes(byte state[4][4]) {
    for (int r = 0; r < 4; r++)
        for (int c = 0; c < 4; c++)
            state[r][c] = inv_sbox[state[r][c]];
}

void InvShiftRows(byte state[4][4]) {
    byte temp;

    // Row 1: shift right by 1
    temp = state[1][3];
    for (int c = 3; c > 0; c--) state[1][c] = state[1][c - 1];
    state[1][0] = temp;

    // Row 2: shift right by 2
    std::swap(state[2][0], state[2][2]);
    std::swap(state[2][1], state[2][3]);

    // Row 3: shift right by 3 (left by 1)
    temp = state[3][0];
    for (int c = 0; c < 3; c++) state[3][c] = state[3][c + 1];
    state[3][3] = temp;
}

void InvMixColumns(byte state[4][4]) {
    for (int c = 0; c < 4; c++) {
        byte a0 = state[0][c];
        byte a1 = state[1][c];
        byte a2 = state[2][c];
        byte a3 = state[3][c];

        state[0][c] = gmul(a0,0x0E) ^ gmul(a1,0x0B) ^ gmul(a2,0x0D) ^ gmul(a3,0x09);
        state[1][c] = gmul(a0,0x09) ^ gmul(a1,0x0E) ^ gmul(a2,0x0B) ^ gmul(a3,0x0D);
        state[2][c] = gmul(a0,0x0D) ^ gmul(a1,0x09) ^ gmul(a2,0x0E) ^ gmul(a3,0x0B);
        state[3][c] = gmul(a0,0x0B) ^ gmul(a1,0x0D) ^ gmul(a2,0x09) ^ gmul(a3,0x0E);
    }
}

/* ============================================================
   KEY EXPANSION (SAME AS ENCRYPTION)
   ============================================================ */

static const byte rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

void KeyExpansion(const byte key[16], byte roundKeys[176]) {
    memcpy(roundKeys, key, 16);

    byte temp[4];
    int bytesGenerated = 16;
    int rconIter = 1;

    while (bytesGenerated < 176) {
        for (int i = 0; i < 4; i++)
            temp[i] = roundKeys[bytesGenerated - 4 + i];

        if (bytesGenerated % 16 == 0) {
            // RotWord
            byte t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;

            // SubWord (using inverse is WRONG — encryption sbox needed normally,
            // but key schedule remains same as encryption)
            static const byte sbox[256] = {
                
            };

            for (int i = 0; i < 4; i++)
                temp[i] = sbox[temp[i]];

            temp[0] ^= rcon[rconIter++];
        }

        for (int i = 0; i < 4; i++) {
            roundKeys[bytesGenerated] =
                roundKeys[bytesGenerated - 16] ^ temp[i];
            bytesGenerated++;
        }
    }
}

/* ============================================================
   AES-128 DECRYPTION
   ============================================================ */

void AESDecrypt(byte input[16], byte output[16], const byte roundKeys[176]) {
    byte state[4][4];

    // Load ciphertext into state
    for (int i = 0; i < 16; i++)
        state[i % 4][i / 4] = input[i];

    AddRoundKey(state, roundKeys + 160);

    for (int round = Nr - 1; round > 0; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, roundKeys + round * 16);
        InvMixColumns(state);
    }

    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, roundKeys);

    // Copy state to output
    for (int i = 0; i < 16; i++)
        output[i] = state[i % 4][i / 4];
}

/* ============================================================
   MAIN
   ============================================================ */

int main() {
    // AES-128 key
    byte key[16] = {
        0x00,0x01,0x02,0x03,
        0x04,0x05,0x06,0x07,
        0x08,0x09,0x0a,0x0b,
        0x0c,0x0d,0x0e,0x0f
    };

    // Ciphertext (from NIST)
    byte ciphertext[16] = {
        0x69,0xc4,0xe0,0xd8,
        0x6a,0x7b,0x04,0x30,
        0xd8,0xcd,0xb7,0x80,
        0x70,0xb4,0xc5,0x5a
    };

    byte plaintext[16];
    byte roundKeys[176];

    // Expand key
    KeyExpansion(key, roundKeys);

    // Decrypt
    AESDecrypt(ciphertext, plaintext, roundKeys);

    // Print result
    std::cout << "Decrypted Plaintext:\n";
    for (int i = 0; i < 16; i++) {
        std::cout << std::hex << std::setw(2)
                  << std::setfill('0')
                  << (int)plaintext[i] << " ";
    }
    std::cout << std::dec << "\n";

    // Expected output check
    byte expected[16] = {
        0x00,0x11,0x22,0x33,
        0x44,0x55,0x66,0x77,
        0x88,0x99,0xaa,0xbb,
        0xcc,0xdd,0xee,0xff
    };

    bool ok = true;
    for (int i = 0; i < 16; i++) {
        if (plaintext[i] != expected[i]) {
            ok = false;
            break;
        }
    }

    if (ok)
        std::cout << "TEST PASSED ✔\n";
    else
        std::cout << "TEST FAILED ✘\n";

    return 0;
}
